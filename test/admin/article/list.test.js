...
describe(chalk.bold.cyan('관리자/게시물 목록'), () => {
  beforeAll(async () => {
    await request.post(`/articles/${boardType}`).send(mockPost);
  });

  describe(`GET /admin/articles/${boardType}`, () => {
    let response;

    beforeAll(async () => {
      response = await request.get(`/admin/articles/${boardType}`);
    });
  ...
  describe('Role 권한 검증', () => {
    test('성공] 요구되는 role을 가진 회원', async () => {
      ...
      const response = await request.get(`/admin/articles/${boardType}`);
