...
describe(chalk.bold.cyan('관리자/팝업 이미지 다운'), () => {
  describe(`GET /admin/popups/img/{imgId}`, () => {
    test(`성공] Status code 200`, async () => {
      const postImgs = await request
        .post(`/admin/popups/img`)
        .set('Connection', 'keep-alive')
        .attach('file', path.join(DUMMY_FILE_PATH, mockPost.fileName));

      const response = await request.get(
        `/admin/popups/img/${postImgs.body.id}`,
      );
    ...
    test(`실패] 존재하지 않는 이미지`, async () => {
      await login(request, 'admin', 'admin');
      const response = await request.get(`/admin/popups/img/abc`);
